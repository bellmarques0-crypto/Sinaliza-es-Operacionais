...
iimport { db } from './src/server/db-supabase';
...
app.get('/api/auth/me', authenticateToken, (req: AuthRequest, res: Response) => {
  const user = db.getUsuarioById(req.user!.id);
  if (!user) {
...
// --- SINALIZAÇÕES ROUTES ---
app.get('/api/sinalizacoes', authenticateToken, (req: Request, res: Response) => {
  const { dataInicial, dataFinal, supervisor, operador, produto, motivo } = req.query;

  let list = db.getSinalizacoes();
...
app.post(
  ...
  (req: AuthRequest, res: Response) => {
    try {
      ...
      const newRecord = db.addSinalizacao({
        data: currentDate,
        hora: currentTime,
        operador,
        supervisor,
...
app.get('/api/dashboard', authenticateToken, (req: Request, res: Response) => {
  ...
  const totalMotivosCadastrados = db.getMotivos().length;
...
app.get(
  ...
  requireRole(['Administrador']),
  (req: Request, res: Response) => {
    const list = db.getUsuarios().map(({ senha, ...u }) => u);
    return res.json(list);
  }
);
...
app.put(
  ...
  (req: Request, res: Response) => {
    ...
    const existing = db.getUsuarioById(id);
    if (!existing) {
      return res.status(404).json({ error: 'Usuário não encontrado.' });
    }
...
app.put(
  ...
  (req: Request, res: Response) => {
    ...
    const updated = db.updateUsuario(id, { senha: hashedPassword });
...
// --- SUPERVISORES ROUTES ---
app.get('/api/supervisores', authenticateToken, (req: Request, res: Response) => {
  return res.json(db.getSupervisores());
});
...
app.put(
  ...
  (req: Request, res: Response) => {
    ...
    const updated = db.updateSupervisor(id, {
      ...(nome && { nome }),
      ...(produto && { produto }),
      ...(status && { status })
    });
...
// --- OPERADORES ROUTES ---
app.get('/api/operadores', authenticateToken, (req: Request, res: Response) => {
  return res.json(db.getOperadores());
});

// --- PRODUTOS ROUTES ---
app.get('/api/produtos', authenticateToken, (req: Request, res: Response) => {
  return res.json(db.getProdutos());
});
...
app.put(
  ...
  (req: Request, res: Response) => {
    ...
    const { nome } = req.body;
    if (!nome) return res.status(400).json({ error: 'Nome do produto é obrigatório.' });
    const updated = db.updateProduto(id, nome);
...
// --- MOTIVOS ROUTES ---
app.get('/api/motivos', authenticateToken, (req: Request, res: Response) => {
  return res.json(db.getMotivos());
});
...
app.post(
  ...
  (req: Request, res: Response) => {
    ...
    const newMotivo = db.addMotivo(descricao);
...
app.put(
  ...
  (req: Request, res: Response) => {
    ...
    const { descricao } = req.body;
    if (!descricao) return res.status(400).json({ error: 'Descrição é obrigatória.' });
    const updated = db.updateMotivo(id, descricao);
...
app.post(
  ...
  (req: Request, res: Response) => {
    ...
    const updated = db.updateConfigApi({
      url_api,
      token,
      usuario,
      ...(senha && senha !== '••••••••••••' && { senha })
    ...
    try {
      ...
      if (apiData) {
        ...
        if (Array.isArray(sups)) {
          for (const s of sups) {
            if (s.nome) {
              const existingSups = db.getSupervisores();
        ...
        if (Array.isArray(ops)) {
          for (const o of ops) {
            ...
            if (nomeOp) {
              const existingOps = db.getOperadores();
      ...
      db.updateConfigApi({ ultima_sincronizacao: formattedDate });
